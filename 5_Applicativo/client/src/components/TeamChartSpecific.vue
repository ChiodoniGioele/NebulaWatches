<!--
  Line graph for visualization of team sales
-->
<template>
...
async function fetchValueForMonth(id, month) {
    try {
        const response = await axios.get(`${apiServerAddress}/v1/team/countSoldMonth/${id}/${month}`,
            {
                headers: {
...
async function getNames() {
    ...
    for (var i = 0; i < teams.value.length; i++) {
        ...
        soldWatches.push(await getDataForName(teams.value[i].id));
    }
